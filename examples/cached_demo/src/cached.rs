...
//! the outer's texture.
//!
//! [`CompositingMode::Bake`] preserves the inline behavior: the
...
/// animations.
// The example exercises `Auto`; the other policies are part of the
// widget's full API and kept for reference.
#[allow(dead_code)]
    ...
    Renderer: renderer::Renderer,
{
    ...
    /// to preserve inline source-order semantics at the cost of correct
    ...
    Renderer: renderer::Renderer,
{
    ...
    fn draw(
        ...
        let at_rest = if layer_mode {
            ...
            // Bake mode keeps the inline semantics: draw both reads and
